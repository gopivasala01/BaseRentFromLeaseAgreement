...
public class Hawaii_Format1 
{
	...
	//public static void main(String args[]) throws Exception
	{
	    ...
	    try
	    {
	    	PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_commencementDate_Prior)+PDFAppConfig.Hawaii_Format1.AB_commencementDate_Prior.length());
	    	PDFReader.commencementDate = PDFReader.commencementDate.substring(0, PDFReader.commencementDate.indexOf(PDFAppConfig.Hawaii_Format1.AB_commencementDate_After)).trim();
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_commencementDate_Prior2)+PDFAppConfig.Hawaii_Format1.AB_commencementDate_Prior2.length());
	   ...
	   try
	    {
		   String expirationDateWaw = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_expirationDate_Prior)+PDFAppConfig.Hawaii_Format1.AB_expirationDate_Prior.length());
		   PDFReader.expirationDate =expirationDateWaw.substring(0,expirationDateWaw.indexOf(PDFAppConfig.Hawaii_Format1.AB_expirationDate_After)).trim();
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		 String expirationDateWaw = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_expirationDate_Prior2)+PDFAppConfig.Hawaii_Format1.AB_expirationDate_Prior2.length());
	   ...
	   try
	    {
		    PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_proratedRent_Prior,1)+PDFAppConfig.Hawaii_Format1.AB_proratedRent_Prior.length()).trim().split("as prorated rent")[0];
	   ...
	   System.out.println("Prorated Rent = "+PDFReader.proratedRent);
	    try
	    {
		    PDFReader.proratedRentDate = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_proratedRentDate_Prior)+PDFAppConfig.Hawaii_Format1.AB_proratedRentDate_Prior.length(),text.indexOf(PDFAppConfig.Hawaii_Format1.AB_proratedRentDate_After)).trim();
	    }
	    catch(Exception e)
	    {
	    ...
	    try
	    {
		    PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_fullRentDate_Prior)+PDFAppConfig.Hawaii_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.Hawaii_Format1.AB_fullRentDate_After));
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format1.AB_fullRentDate_Prior)+PDFAppConfig.Hawaii_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.Hawaii_Format1.AB_fullRentDate1_After));
			   	System.out.println("Monthly Rent Date= "+PDFReader.monthlyRentDate.trim());
	    	}
	    	catch(Exception e1)
		    {
	  ...
	  //Increased Rent Check
	    try
	    {
	    	if(PDFReader.monthlyRent.contains("*")||text.contains(PDFAppConfig.Hawaii_Format2.monthlyRentAvailabilityCheck)==true)
	    	{
	    		...
	    		//if(endDateArray[2].trim().length()==4)//&&RunnerClass.onlyDigits(endDateArray[2]))
	    		 //{
	    		  ...
	    		  String newRentStartDate[] = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format2.increasedRent_newStartDate_Prior)+PDFAppConfig.Hawaii_Format2.increasedRent_newStartDate_Prior.length()).trim().split(" ");
	    		  ...
	    		  String increasedRentRaw = text.substring(text.indexOf(PDFAppConfig.Hawaii_Format2.increasedRent_newStartDate_Prior)+PDFAppConfig.Hawaii_Format2.increasedRent_newStartDate_Prior.length()).trim();
