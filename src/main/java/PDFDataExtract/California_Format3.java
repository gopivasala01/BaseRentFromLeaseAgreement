...
public class California_Format3 
{
	public static String text ="";
	public static  boolean  format3() throws Exception
	//public static void main(String[] args) throws Exception 
	{
	    ...
	    try
	    {
	    	String commensementRaw = text.substring(text.indexOf(PDFAppConfig.California_Format3.commensementDate_Prior)+PDFAppConfig.California_Format3.commensementDate_Prior.length()+1).trim();//,text.indexOf(PDFAppConfig.California_Format2.commensementDate_After)).trim();
	    	 PDFReader.commencementDate = commensementRaw.substring(0, commensementRaw.indexOf('.')).trim();
	    	 PDFReader.commencementDate = PDFReader.commencementDate.trim().replaceAll(" +", " ").replaceAll("[\\p{Z}\\s]+", "");
	    ...
	    try
	    {
	    	String expirationDateRaw  = text.substring(text.indexOf(PDFAppConfig.California_Format3.expirationDate_Prior)+PDFAppConfig.California_Format3.expirationDate_Prior.length()).trim();//,text.indexOf(PDFAppConfig.California_Format2.expirationDate_After)).trim();
	    	PDFReader.expirationDate = expirationDateRaw.substring(0,expirationDateRaw.indexOf('.')).trim();
	    	PDFReader.expirationDate = PDFReader.expirationDate.trim().replaceAll(" +", " ").replaceAll("[\\p{Z}\\s]+", "");
	    ...
	    /*try
	    {
	    	PDFReader.proratedRentDate = text.substring(text.indexOf(PDFAppConfig.California_Format2.proratedRentDate_Prior)+PDFAppConfig.California_Format2.proratedRentDate_Prior.length()+1,text.indexOf(PDFAppConfig.California_Format2.proratedRentDate_After)).trim();
	    ...
	    try
	    {
	    	PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.California_Format2.proratedRent_Prior)+PDFAppConfig.California_Format2.proratedRent_Prior.length()).split(" ")[0].trim();
	    ...
	    */
	    try
	    {
	    	...
	    	String monthlyRentraw = text.substring(text.indexOf(PDFAppConfig.California_Format3.monthlyRent_Prior)+PDFAppConfig.California_Format3.monthlyRent_Prior.length()).split("per")[0].trim();
	    	PDFReader.monthlyRent = monthlyRentraw.replaceAll("[\\s$]+","");
	    	if(!PDFReader.monthlyRent.contains("."))
	    		PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.California_Format3.monthlyRent_Prior2)+PDFAppConfig.California_Format3.monthlyRent_Prior2.length()).split(" ")[0].trim();
	    	if(PDFReader.monthlyRent.matches(".*[a-zA-Z]+.*"))
		    {
		    	PDFReader.monthlyRent = "Error";
		    }
	    	if(PDFReader.monthlyRent.contains("*")||text.contains(PDFAppConfig.California_Format3.monthlyRentAvailabilityCheck)==true)
	    	{
	    		...
	    		if(endDateArray[2].trim().length()==4)//&&RunnerClass.onlyDigits(endDateArray[2]))
	    		 {
	    		  ...
	    		  String newRentStartDate[] = text.substring(text.indexOf(PDFAppConfig.California_Format3.increasedRent_newStartDate_Prior)+PDFAppConfig.California_Format3.increasedRent_newStartDate_Prior.length()).trim().split(" ");
	    		  ...
	    		  String increasedRentRaw = text.substring(text.indexOf(PDFAppConfig.California_Format3.increasedRent_newStartDate_Prior)+PDFAppConfig.California_Format3.increasedRent_newStartDate_Prior.length()).trim();
