...
public class SouthCarolina_Format1 
{
	...
	//public static void main(String args[]) throws Exception
	{
	    ...
	    try
	    {
	    	PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_commencementDate_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_commencementDate_Prior.length());
	    	PDFReader.commencementDate = PDFReader.commencementDate.substring(0, PDFReader.commencementDate.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_commencementDate_After)).trim();
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.commencementDate = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_commencementDate_Prior2)+PDFAppConfig.SouthCarolina_Format1.AB_commencementDate_Prior2.length());
	   ...
	   try
	    {
		   String expirationDateWaw = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_expirationDate_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_expirationDate_Prior.length());
		   PDFReader.expirationDate =expirationDateWaw.substring(0,expirationDateWaw.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_expirationDate_After)).trim();
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		 String expirationDateWaw = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_expirationDate_Prior2)+PDFAppConfig.SouthCarolina_Format1.AB_expirationDate_Prior2.length());
	   ...
	   try
	    {
		    PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_proratedRent_Prior,1)+PDFAppConfig.SouthCarolina_Format1.AB_proratedRent_Prior.length()).trim().split(" ")[0];
	   ...
	   System.out.println("Prorated Rent = "+PDFReader.proratedRent);
	    try
	    {
		    PDFReader.proratedRentDate = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_proratedRentDate_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_proratedRentDate_Prior.length(),text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_proratedRentDate_After)).trim();
	    }
	    catch(Exception e)
	    {
	    ...
	    try
	    {
		    PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRentDate_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRentDate_After));
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRentDate_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRentDate1_After));
			   	System.out.println("Monthly Rent Date= "+PDFReader.monthlyRentDate.trim());
	    	}
	    	catch(Exception e1)
		    {
	    ...
	    try
	    {
		    PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRent_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_fullRent_Prior.length()).trim().split(" ")[0].trim();//,text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRent_After)).substring(1).replaceAll("[^.0-9]", "");;
		    if(RunnerClass.onlyDigits(PDFReader.monthlyRent.replace(".", "").replace(",", ""))==false)
		    {
		    	PDFReader.monthlyRent = text.substring(text.indexOf(PDFAppConfig.SouthCarolina_Format1.AB_fullRent2_Prior)+PDFAppConfig.SouthCarolina_Format1.AB_fullRent2_Prior.length()).trim().split(" ")[0].trim();
		    }
