...
public class Alabama_Format1 
{
	...
	//public static void main(String args[]) throws Exception
	{
	   ...
	   try
	    {
		    PDFReader.proratedRent = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_proratedRent_Prior,1)+PDFAppConfig.Alabama_Format1.AB_proratedRent_Prior.length()).trim().split(" ")[0];
	   ...
	   //System.out.println("Prorated Rent = "+PDFReader.proratedRent);
	    try
	    {
		    PDFReader.proratedRentDate = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_proratedRentDate_Prior)+PDFAppConfig.Alabama_Format1.AB_proratedRentDate_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_proratedRentDate_After)).trim();
	    }
	    catch(Exception e)
	    {
	    ...
	    try
	    {
		    PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_fullRentDate_Prior)+PDFAppConfig.Alabama_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_fullRentDate_After));
	    ...
	    catch(Exception e)
	    {
	    	try
	    	{
	    		PDFReader.monthlyRentDate = text.substring(text.indexOf(PDFAppConfig.Alabama_Format1.AB_fullRentDate_Prior)+PDFAppConfig.Alabama_Format1.AB_fullRentDate_Prior.length(),text.indexOf(PDFAppConfig.Alabama_Format1.AB_fullRentDate1_After));
			   	System.out.println("Monthly Rent Date= "+PDFReader.monthlyRentDate.trim());
	    	}
	    	catch(Exception e1)
		    {
